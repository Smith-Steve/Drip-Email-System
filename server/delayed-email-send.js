...
const sqlEmailGetQuery = `select DISTINCT on ("e"."emailId") "f"."flightName" as "flightName", "e"."emailId" as "emailId", "s"."scriptName", "e"."subject", "e"."emailBody", "f"."flightId", "e"."scriptId",
...
db.query(sqlEmailGetQuery)
  ...
  .then(() => {
    ...
    db.query(sqlFlightCompleteQuery, param)
      .then(result => {
        const returnedInfo = result.rows[0];
        if (!returnedInfo.flightComplete) {
          const sqlUpdateQuery = `update "flights"
             set "flightComplete" = true
             where "flightId" = $1`;
          // console.log(module.flightInformation.flightId);
          const param = [module.flightInformation.flightId];
          db.query(sqlUpdateQuery, param)
            .catch(err => {
              console.error(err);
            });
        }
